...
function CM_Martha_Character:CheckPlayerData(player)
	local player_type = player:GetPlayerType()
	if player_type == MarthaPlayerType.PLAYER_MARTHA then
		None:TryAddSimulatedSpell(player, SpellType.SPELL_HEAVEN_RAY)
	else
		None:TryRemoveSimulatedSpell(player, sim_spell_ID)
	end
end
ModRef:AddPriorityCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, CallbackPriority.EARLY, CM_Martha_Character.CheckPlayerData, 0)

function CM_Martha_Character:HopeChargeDataInit(player)
	local player_type = player:GetPlayerType()
...
ModRef:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, CM_Martha_Character.HopeChargeDataInit, 0)
...
function CM_Martha_Character:MPIconOffsetUpdate(player)
...
ModRef:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, CM_Martha_Character.MPIconOffsetUpdate, 0)

function CM_Martha_Character:EID_OnRender()
	if EID then
...
ModRef:AddCallback(ModCallbacks.MC_POST_RENDER, CM_Martha_Character.EID_OnRender)

function CM_Martha_Character:HopeChargeDataUpdate(player)
	local player_type = player:GetPlayerType()
	local key = "HopeCharge"
...
ModRef:AddPriorityCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, CallbackPriority.LATE, CM_Martha_Character.HopeChargeDataUpdate, 0)

function CM_Martha_Character:HopeChargeDataDecrease(player, attribute_name, amount)
	local player_type = player:GetPlayerType()
...
ModRef:AddPriorityCallback(tbomCallbacks.TBOMC_POST_COST_SPCHARGE, CallbackPriority.IMPORTANT, CM_Martha_Character.HopeChargeDataDecrease)

function CM_Martha_Character:HeavenRay_OnUse(sim_spell_ID, rng, player, use_flag)
	...
	local texts = {["en"] = "Heaven Ray", ["zh"] = "ÃÏΩÁ…‰œﬂ"}
...
ModRef:AddCallback(tbomCallbacks.TBOMC_POST_USE_SIM_SPELL, CM_Martha_Character.HeavenRay_OnUse, SpellType.SPELL_HEAVEN_RAY)

return CM_Martha_Character